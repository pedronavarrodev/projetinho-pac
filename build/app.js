class Ghost {
  ...
  setMovementStats(pacman, name, level) {
    ...
    this.slowSpeed = pacmanSpeed * (0.70 + levelAdjustment); // 0.75  estava esse valor
    this.mediumSpeed = pacmanSpeed * (0.80 + levelAdjustment); // 0.875 estava esse valor
    this.fastSpeed = pacmanSpeed * (1 + levelAdjustment); // 1 estava esse valor
...
class GameCoordinator {
    ...
    mazeArray.forEach((row, rowIndex) => {
      row.forEach((block, columnIndex) => {
        if (block === 'o' || block === 'O') {
          ...
          const points = (block === 'o') ? 0.01 : 0.09;
  ...
  determineComboPoints() {
    return (1.33 * (1.25 ** this.ghostCombo));
  }
